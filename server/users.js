...
function updateProfile(req, res, next) {
    ...
    db.query('update salesforce.contact SET firstName=$1, lastName=$2, mobilePhone=$3, pictureURL__c=$4, preference__c=$5, size__c=$6, emailpref__c=$8, smspref__c=$9, offerpref__c=$10,marketingpref__c=$11 WHERE id=$7',
            [user.firstname, user.lastname, user.mobilephone, user.pictureurl, user.preference, user.size, userId, user.emailpref,user.smspref,user.offerpref,user.marketingpref])
        .then(function () {
            res.send(user);
        })
        .catch(next);
